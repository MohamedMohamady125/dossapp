...
router.post('/', asyncHandler(async (req: AuthRequest, res) => {
  ...
  if (!from || !to) { res.status(400).json({ error: 'Pickup and destination locations are required' }); return; }
  ...
  const activeRide = await dbGet(`SELECT id FROM rides WHERE passenger_id = ? AND status IN ('pending', 'accepted', 'arriving', 'picked_up')`, [userId]);
  if (activeRide) { res.status(409).json({ error: 'You already have an active ride request' }); return; }

  const result = await dbRun(`INSERT INTO rides (passenger_id, from_location, to_location, passenger_count, status) VALUES (?, ?, ?, ?, 'pending')`, [userId, from, to, count]);
...
router.get('/active', asyncHandler(async (req: AuthRequest, res) => {
  ...
  const ride = await dbGet(
    `SELECT r.*, p.name as passenger_name, p.phone as passenger_phone, d.name as driver_name, d.phone as driver_phone, d.vehicle_number
     ...
     WHERE ${condition} AND r.status IN ('pending', 'accepted', 'arriving', 'picked_up') ORDER BY r.created_at DESC LIMIT 1`, [userId]);
  res.json(ride ? formatRide(ride) : null);
}));

router.get('/pending', requireDriver, asyncHandler(async (_req, res) => {
...
router.post('/:id/accept', requireDriver, asyncHandler(async (req: AuthRequest, res) => {
  const rideId = parseInt(req.params.id, 10);
  const driverId = req.user!.userId;

  const ride = await dbGet('SELECT * FROM rides WHERE id = ?', [rideId]);
  if (!ride) { res.status(404).json({ error: 'Ride not found' }); return; }
  if (ride.status !== 'pending') { res.status(409).json({ error: 'This ride is no longer available' }); return; }

  const activeDriverRide = await dbGet(`SELECT id FROM rides WHERE driver_id = ? AND status IN ('accepted', 'arriving', 'picked_up')`, [driverId]);
  if (activeDriverRide) { res.status(409).json({ error: 'You already have an active ride in progress' }); return; }

  await dbRun(`UPDATE rides SET driver_id = ?, status = 'accepted', eta_minutes = 3, updated_at = datetime('now') WHERE id = ?`, [driverId, rideId]);
  await dbRun("UPDATE users SET is_available = 0, updated_at = datetime('now') WHERE id = ?", [driverId]);

  const updated = await dbGet(
    `SELECT r.*, p.name as passenger_name, p.phone as passenger_phone, d.name as driver_name, d.phone as driver_phone, d.vehicle_number
     FROM rides r JOIN users p ON r.passenger_id = p.id LEFT JOIN users d ON r.driver_id = d.id WHERE r.id = ?`, [rideId]);
  res.json(formatRide(updated));
}));

router.post('/:id/status', asyncHandler(async (req: AuthRequest, res) => {
  const rideId = parseInt(req.params.id, 10);
  const { status } = req.body;
  const userId = req.user!.userId;

  const validTransitions: Record<string, string[]> = {
    accepted: ['arriving', 'cancelled'], arriving: ['picked_up', 'cancelled'],
    picked_up: ['arrived', 'cancelled'], arrived: ['completed'],
  };

  const ride = await dbGet('SELECT * FROM rides WHERE id = ?', [rideId]);
  if (!ride) { res.status(404).json({ error: 'Ride not found' }); return; }
  if (Number(ride.passenger_id) !== userId && Number(ride.driver_id) !== userId) { res.status(403).json({ error: 'Not authorized to update this ride' }); return; }

  const allowed = validTransitions[ride.status as string];
  if (!allowed || !allowed.includes(status)) { res.status(400).json({ error: `Cannot transition from '${ride.status}' to '${status}'` }); return; }
...
router.post('/:id/cancel', asyncHandler(async (req: AuthRequest, res) => {
  ...
  const ride = await dbGet('SELECT * FROM rides WHERE id = ?', [rideId]);
  if (!ride) { res.status(404).json({ error: 'Ride not found' }); return; }
  if (Number(ride.passenger_id) !== userId && Number(ride.driver_id) !== userId) { res.status(403).json({ error: 'Not authorized to cancel this ride' }); return; }
  if (['completed', 'cancelled'].includes(ride.status as string)) { res.status(400).json({ error: 'Ride is already finished' }); return; }
...
router.get('/history', asyncHandler(async (req: AuthRequest, res) => {
  const userId = req.user!.userId;
  const condition = req.user!.isDriver ? 'r.driver_id = ?' : 'r.passenger_id = ?';
  const rides = await dbAll(
    `SELECT r.*, p.name as passenger_name, p.phone as passenger_phone, d.name as driver_name, d.phone as driver_phone, d.vehicle_number
     FROM rides r JOIN users p ON r.passenger_id = p.id LEFT JOIN users d ON r.driver_id = d.id
     WHERE ${condition} AND r.status IN ('completed', 'cancelled') ORDER BY r.updated_at DESC LIMIT 50`, [userId]);
  res.json(rides.map(formatRide));
}));
