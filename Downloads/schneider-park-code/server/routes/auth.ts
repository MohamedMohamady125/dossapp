...
router.post('/register', async (req: AuthRequest, res: Response) => {
  ...
  if (!lowerEmail.endsWith('@schneider-electric.com') && !lowerEmail.endsWith('@se.com')) {
    res.status(400).json({ error: 'Only Schneider Electric emails (@schneider-electric.com or @se.com) are allowed' });
    return;
  }
...
router.post('/login', async (req: AuthRequest, res: Response) => {
  ...
  if (!user.is_verified) {
    ...
    await dbRun(
      `INSERT INTO verification_codes (user_id, code, destination, expires_at) VALUES (?, ?, ?, ?)`,
      [user.id, code, destination as string, expiresAt]
    );
...
router.post('/verify', async (req: AuthRequest, res: Response) => {
  ...
  const record = await dbGet(
    `SELECT * FROM verification_codes WHERE user_id = ? AND code = ? AND used = 0 ORDER BY created_at DESC LIMIT 1`,
    [userId, code]
  );
...
router.post('/resend-code', async (req: AuthRequest, res: Response) => {
  ...
  await dbRun(
    `INSERT INTO verification_codes (user_id, code, destination, expires_at) VALUES (?, ?, ?, ?)`,
    [userId, code, destination as string, expiresAt]
  );
