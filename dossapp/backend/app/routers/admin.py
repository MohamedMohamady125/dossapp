...
async def debug_attendance(branch_id: int, admin: AdminUser = Depends(get_current_admin)):
    """Temporary: inspect attendance parsing for a branch."""
    from app.main import roster_source
    ...
    try:
        ...
        for sn in wb.sheetnames:
            ...
            if not is_att:
                continue
            ws = wb[sn]
            # Dump raw header rows
            headers = []
            for r in range(1, min(6, (ws.max_row or 0) + 1)):
                cells = {}
                for col in range(1, min(60, (ws.max_column or 0) + 1)):
            ...
            sample_rows = []
            for r in range(6, min(15, (ws.max_row or 0) + 1)):
                cells = {}
                for col in range(1, min(60, (ws.max_column or 0) + 1)):
                    v = ws.cell(row=r, column=col).value
                    if v is not None:
                        cells[f"C{col}"] = f"{type(v).__name__}:{repr(v)}"
                if cells:
                    sample_rows.append({"row": r, "cells": cells})

            result["attendance_sheets"].append({
                "name": sn,
                "max_row": ws.max_row,
                "max_col": ws.max_column,
                "headers": headers,
                "sample_data_rows": sample_rows,
                "schedule_count": len(sched_map),
                "extras_count": len(extras_map),
                "attendance_athletes": len(att_map),
                "total_marks": sum(len(v) for v in att_map.values()),
                "sample_attendance": {str(k): v for k, v in list(att_map.items())[:3]},
                "errors": errs,
            })
