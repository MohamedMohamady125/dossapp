...
def scrape_fastest_times(driver, team_url, gender="M"):
    """Scrape fastest times for each event
    ...
    try:
        ...
        print(f"    Fetching {gender} rankings from: {rankings_url}")
        ...
        for header in event_headers:
            ...
            # Find the next row (first swimmer = fastest time)
            next_row = header.find_parent("tr").find_next_sibling("tr")
            ...
            if next_row:
                ...
                if len(cells) >= 3:
                    ...
                    # Store with gender prefix
                    gender_prefix = "mens" if gender == "M" else "womens"
                    event_key = f"{gender_prefix}_{event_name.replace(' ', '_').lower()}"

                    times_data[event_key] = time_value
                    times_data[f"{event_key}_swimmer"] = swimmer_name
                    times_data[f"{event_key}_meet"] = meet_name

        print(f"    ✓ Collected {len([k for k in times_data.keys() if not k.endswith('_swimmer') and not k.endswith('_meet')])} {gender} event times")
        ...
        for card in coach_cards:
            ...
            if media_body:
                ...
                if name_elem:
                    ...
                    # Search for email in text and HTML
...
def scrape_team_enhanced(driver, team_info):
    ...
    # 2. Men's fastest times
    print("  [2/4] Men's fastest times...")
    mens_times = scrape_fastest_times(driver, team_info['url'], gender="M")
    all_data.update(mens_times)

    # 3. Women's fastest times
    print("  [3/4] Women's fastest times...")
    womens_times = scrape_fastest_times(driver, team_info['url'], gender="F")
...
def main():
    print("="*60)
    print("ENHANCED SCRAPER - TEST ON 2 SCHOOLS")
    print("="*60)
    ...
    if all_team_data:
        ...
        with open("teams_enhanced_test.csv", "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            for team in all_team_data:
                writer.writerow(team)

        print(f"✓ Saved {len(all_team_data)} teams to teams_enhanced_test.csv")
        print(f"✓ Total fields: {len(fieldnames)}\n")

        print("Sample fields captured:")
        # Show some interesting fields
        sample_fields = [f for f in sorted(fieldnames) if 'mens' in f or 'womens' in f or 'coach' in f][:20]
