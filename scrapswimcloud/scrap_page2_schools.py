...
def scrape_page_2_teams(driver, base_url, gender="M"):
    """
    Scrape teams from page 2 only
    gender: "M" for men, "F" for women
    """
    teams = []
    page = 2  # Fixed to page 2

    print(f"\nScraping {'Men' if gender == 'M' else 'Women'}'s Teams from Page 2:")
    print("-" * 80)

    # Update URL with gender parameter and page 2
    ...
    for row in rows:
        ...
        if len(cells) >= 1:
            ...
            if team_link:
                ...
                # Calculate rank (51-100 for page 2)
                rank = 50 + len(teams) + 1
...
def main():
    print("\n" + "="*80)
    print("PAGE 2 SCRAPER - SCHOOLS 51-100")
    print("="*80)

    # URL for page 2
    TEAMS_URL = "https://www.swimcloud.com/country/usa/teams/?eventCourse=Y&gender=M&page=2&rankType=D&region=division_1&seasonId=29&sortBy=all"

    print(f"\nConfiguration:")
    print(f"  - Scraping: Page 2 only (Schools 51-100)")
    ...
    try:
        print("\n" + "="*80)
        print("STEP 1: SCRAPING PAGE 2 TEAMS BY GENDER")
        print("="*80)

        # Scrape men's teams from page 2
        mens_teams = scrape_page_2_teams(driver, TEAMS_URL, gender="M")

        # Scrape women's teams from page 2
        womens_teams = scrape_page_2_teams(driver, TEAMS_URL, gender="F")
        ...
        output_file = "page2_schools_rankings.csv"
        fieldnames = [
    ...
    print("ALL DONE! 🏊‍♂️🏊‍♀️")
    print("="*80)
    print(f"\nYour data is ready in: page2_schools_rankings.csv")
    print("="*80 + "\n")
